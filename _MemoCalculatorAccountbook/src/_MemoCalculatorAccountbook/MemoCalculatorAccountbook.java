...
public class MemoCalculatorAccountbook extends JFrame {
	...
	public MemoPanel panel2;
	public Account panel3;
	...
	MemoCalculatorAccountbook() {
		...
		Panel1 = new CalculatorPanel();
		panel2 = new MemoPanel(); // 대충 뭐 메모장 계산기 이런거 들어감
		panel3 = new Account();
		// setContentPane(contentPane);
		tabbedPane.add("계산기", Panel1);
	...
	class CalculatorPanel extends JPanel {
            ...
            for (Integer i = 0; i < 10; i++) {
               if (e.getSource().equals(numberBtn[i])) {
                  ...
                  if (inputTfString.equals("")) // 재입력 시작순간 폰트 크기 초기화
                     inputTf.setFont(new Font(null, Font.PLAIN, 22)); // 폰트크기초기화
      ...
      class AdjustActionListener implements ActionListener {
         public void actionPerformed(ActionEvent e) {
            if (e.getSource().equals(ACBtn)) {
               cal.initAllElement(); // cal객체의 number1 number2 result 0으로 초기화
               initInputTfString();
               setInputTfResult(cal); // inputTf 텍스트 필드에 result를 출력, 0이 출력
               //calPushBtn = false;
               inputTf.setFont(new Font(null, Font.PLAIN, 22)); // 폰트 크기초기화
      ...
      class PKActionListener implements ActionListener {
         public void actionPerformed(ActionEvent e) {
            if (e.getSource().equals(PBtn)){
               //calPushBtn=true;
               cal.stringToCalNum(inputTf.getText());
               cal.setSymbol('p'); // 각 버튼에 맞는 연산 기호를 입력
               setInputTfResult(cal);
               }
            else if (e.getSource().equals(KBtn)){
               cal.stringToCalNum(inputTf.getText());
               cal.setSymbol('k');
               setInputTfResult(cal);
            }
      ...
      class ICActionListener implements ActionListener {
         public void actionPerformed(ActionEvent e) {
            if (e.getSource().equals(InBtn)){
               //calPushBtn=true;
               cal.stringToCalNum(inputTf.getText());
               cal.setSymbol('i'); // 각 버튼에 맞는 연산 기호를 입력
               setInputTfResult(cal);
               }
            else if (e.getSource().equals(cmBtn)){
               cal.stringToCalNum(inputTf.getText());
               cal.setSymbol('m');
               setInputTfResult(cal);
            }
      ...
      class CFActionListener implements ActionListener {
         // double cels = Float.valueOf(inputTf.getText());
         // double fah = Float.valueOf(inputTf.getText());
         ...
         public void actionPerformed(ActionEvent e) {
            ...
            if (e.getSource().equals(CBtn)){
               //calPushBtn=true;
               cal.stringToCalNum(inputTf.getText());
               cal.setSymbol('c'); // 각 버튼에 맞는 연산 기호를 입력
               setInputTfResult(cal);
               }
      ...
      class CalculateActionListener implements ActionListener {
         public void actionPerformed(ActionEvent e) {
            if (calPushBtn && !e.getSource().equals(equalBtn)) // calPushBtn이 true 면서
               ...
               return;
            cal.stringToCalNum(inputTf.getText()); // 텍스트 필드에 있는 문자열을cal.calNum에 대입
